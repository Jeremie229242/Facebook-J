...
const ProfileHeader = ({
  ...
  const onSubmitCoverPhoto = async (e) => {
    ...
    try {
      ...
      console.error("Erreur lors de la mise à jour de la photo de couverture de l'utilisateur", error);
    } finally {
      setLaoding(false);
    }
  };
      ...
      <div className="relative h-64 md:h-80 bg-gray-300 overflow-hidden ">
        ...
        {isOwner && (
          ...
          >
            ...
            <span className="hidden md:block">Modifier photo de couverture</span>
          </Button>
        )}
      </div>
      {/* profile section */}
        ...
        <div className="flex flex-col md:flex-row items-center md:items-end md:space-x-5 ">
          ...
          <div className="mt-4 mdLmt-0 text-center md:text-left flex-grow">
            ...
            <p className="text-gray-400 font-semibold">
              {profileData?.followerCount} Abbonnées
            </p>
          </div>
          {isOwner && (
            <Button
          ...
          >
            ...
            >
              ...
              <form className="space-y-4" onSubmit={handleSubmit(onSubmitProfile)}>
                ...
                >
                  <Save className="w-4 h-4 mr-2" /> {loading ? "Patienter...": "Sauvegarder"}
                </Button>
              </form>
            </motion.div>
          </motion.div>
              ...
              <form className="space-y-4">
                <div className="flex flex-col items-center mb-4">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={() => coverImageInputRef.current?.click()}>
                    <Upload className="h-4 w-4 mr-2" />
                    Selectionner nouvelle Photo de Profile
                  </Button>
                </div>
                ...
                <Button
                  ...
                  type="button"
                >
                  <Save className="w-4 h-4 mr-2" /> {loading ? "Patienter..." :"Sauvegarder Photo Couverture"}
                </Button>
              </form>
            </motion.div>
          </motion.div>
