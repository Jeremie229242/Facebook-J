...
const Page = () => {
   ...
   const handleAction = async(action,userId) =>{
    if(action === "confirm"){
       toast.success("Abonnée ajouter avec succes")
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-[rgb(36,37,38)] ">
      ...
      <main className="ml-0 md:ml-64 mt-16 p-6">
        <h1 className="text-2xl font-bold mb-6">Demande des Abonnées</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6  ">
          {loading ? (
            <FriendCardSkeleton />
          ) : friendRequest.length === 0 ? (
            <NoFriendsMessage
              text="Pas de demande abbonnement"
              description="On dirait que tu es à jour ! Pourquoi ne pas explorer et rencontrer de nouvelles personnes ?"
            />
          ) : (
